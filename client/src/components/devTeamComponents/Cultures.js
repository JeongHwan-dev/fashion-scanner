  ...
  const members = [
    {
      ...
      position: 'PM & AI 차시현',
      content: `패션 스캐너 크루는 트렌드를 읽고 트렌디한 서비스를 제작합니다. 
      트렌드를 맹목적으로 쫓는 것이 아닌,  사용자가 선호하는 "변하는 가치"로 생각합니다.
      사용자가 만족 할 수 있는 서비스를 끊임없이 고민하여, 트렌드를 알고 싶은 사람이 꼭 방문하는 웹서비스를 최종목표로 합니다.`,
      hashtag: {
        h1: '#Reading Trend',
        h2: '#Leading Trend',
      },
    ...
    },
    {
      ...
      content: `"이게 무슨 코드야? 어떤 로직으로 돌아가는 거야?" 라고 동료 개발자들이 자신에게 묻는 일이 많아진다면 그것은 잘못된 방법으로 개발을 하고 있는 건지도 모릅니다.
      좋은 코드란 타인이 보았을 때도 책을 읽듯 술술 읽히는 코드가 좋은 코드라고 생각하며 이를 위해 리팩토링을 통한 코드 개선에 노력합니다. `,
